...
CREATE TABLE games(id int primary key, game_name varchar(30) not null unique ,no_of_players varchar(30) not null unique, check(no_of_players >10),
...
INSERT INTO games VALUES(1, 'Cricket', 11, 'India', 45, 'virat_kohli', 9, 5, 'vivo', 'Banglore', 1, 8, 10, 'India', 'Australia', 'Australia','Sukha bose', 'India', 'team India', 'Australia', 'International');
INSERT INTO games VALUES(2, 'Football', 12, 'Africa', 50, 'Ronwen williams', 8, 6, 'fan', 'Africa', 3, 12, 12, 'Africa', 'India', 'India','Albert', 'Africa', 'Team Africa', 'Africa', 'National');
INSERT INTO games VALUES(3, 'Kabbaddi', 13, 'Bengalur bulls', 55, 'Pavan', 11, 7, 'vimal', 'Delhi', 2, 9, 13, 'Bengalur bulls', 'Tamil talivas', 'Benagur bulls','Ranveer sharma', 'Brezil', 'Bengalur bulls', 'Tamil talivas', 'International');
INSERT INTO games VALUES(4, 'Badminton', 14, 'Cheena', 40, 'sea-woon', 6, 2, 'oppo', 'Shangai', 4, 18, 11, 'Shehanzai', 'Afghanistan', 'Afghanistan','Rehan ali', 'France', 'team Afghastian', 'Cheena', 'National');
INSERT INTO games VALUES(5, 'Boxing ', 15, 'America', 48, 'Robert', 7, 9, 'renold', 'St.jhons street', 5, 17, 19, 'America', 'Newzeeland', 'Newzeeland','Syed khan', 'Japan', 'team Newzeeland', 'America', 'International');
INSERT INTO games VALUES(6, 'Lawn Bowls', 16, 'Australia', 56, 'Zukeburg', 10, 10, 'redbull', 'machado', 6, 15, 20, 'Australia', 'Cheena', 'Cheena','Xiao un', 'Cheena', 'team Cheena', 'Mexican', 'National');
INSERT INTO games VALUES(7, 'Netball ', 17, 'Pakistan', 85, 'Alladdin', 3, 11, 'stag', 'Alistan', 7, 14, 14, 'Pakistan', 'South-korea', 'Pakistan','Muskan', 'Turki', 'team Pakistan', 'Pakistan', 'International');
INSERT INTO games VALUES(8, 'Rugby 7s', 18, 'Afghanistan', 96, 'Mahammad', 13, 13, 'redmi', 'Zohail', 8, 6, 5, 'Afghanistan', 'Nizeria', 'Nizeria','Sehena', 'Paris', 'team Nizeria', 'Afghanistan', 'International');
INSERT INTO games VALUES(9, 'Hockey', 19, 'Maleshia', 63, 'Rehan', 12, 14, 'cococola', 'Levana', 9, 5, 15, 'Maleshia', 'West-indies', 'West-indies','Crister', 'Jermany', 'team west-indies', 'West-indies', 'National');
INSERT INTO games VALUES(10, 'Weightlifting', 20, 'Nizeria', 32, 'Vicenzo', 16, 12, 'Scotch', 'leggo', 10, 4, 16, 'Nizeria', 'south-indies', 'south-indies','libour', 'Finland', 'team South-indies', 'South-indies', 'International');
INSERT INTO games VALUES(11, 'Squash', 21, 'West-Indies', 47, 'Krona', 19, 4, 'king fisher', 'Stadan', 11, 3, 17, 'West-indies', 'England', 'England','Ghockberk', 'Italy', 'team England', 'England', 'International');
INSERT INTO games VALUES(12, 'Swimming', 22, 'South-Korea', 95, 'Le min ho', 20, 19, 'twinkle', 'Bhagthan', 12, 19, 18, 'South-korea', 'Bhutan', 'Bhutan','Felix', 'Spain', 'team Bhutan', 'Bhutan', 'National');
INSERT INTO games VALUES(13, 'Basketball', 23, 'South-Africa', 23, 'Yanosh', 18, 18, 'lenovo', 'Swtart', 13, 29, 29, 'South-africa', 'USA', 'USA','Cris', 'Greece', 'team USA', 'USA', 'International');
INSERT INTO games VALUES(14, 'Shooting-Pistol', 24, 'NewzeeLand', 75, 'Peter', 29, 17, 'Hp', 'giddo', 14, 16, 27, 'Newzeeland', 'Japan', 'Japan','Asley', 'UK', 'team France', 'France', 'International');
INSERT INTO games VALUES(15, 'Canoeing', 25, 'England', 39, 'Harris', 26, 21, 'Loreal', 'Sannalia', 15, 13, 21, 'England', 'France', 'France','Henry', 'Holand', 'team Holand ', 'Korea', 'National');
...
SELECT lpad('veena', 10, 'AA');
SELECT lpad('veena', 8, 'AA');
SELECT lpad('veena', 3, 'AA');
SELECT lpad('veena', 5, 'AA');
SELECT lpad('veena', 14, 'AA');

/* rpad */
SELECT rpad('veena', 10, 'AA');
SELECT rpad('veena', 15, 'AA');
SELECT rpad('veena', 9, 'AA');
SELECT rpad('veena', 2, 'AA');
SELECT rpad('veena', 6, 'AA');

/* like by */

SELECT * FROM games WHERE host_country LIKE '%i'; 
SELECT * FROM games WHERE captain_name LIKE 'v%'; 
SELECT * FROM games WHERE sponser LIKE 'r%'; 
SELECT * FROM games WHERE venue LIKE 'b%'; 
SELECT * FROM games WHERE country_participated LIKE 's%'; 

 /* reverse */
 
SELECT REVERSE(game_name) FROM games;
SELECT REVERSE(country_participated) FROM games;
SELECT REVERSE(captain_name) FROM games;
SELECT REVERSE(sponser) FROM games;
SELECT REVERSE(venue) FROM games;
SELECT REVERSE(team_rank) FROM games;
...
SELECT * FROM games order by game_name desc;
SELECT * FROM games order by sponser asc;
SELECT * FROM games order by host_country desc;

/* In */

SELECT * FROM games WHERE id IN(1,2,7,8);
SELECT * FROM games WHERE game_name IN('cricket', 'football', 'kabbaddi', 'hockey', 'swimming');
SELECT * FROM games WHERE host_country IN('india', 'america', 'pakistan');
SELECT * FROM games WHERE id IN(1, 13, 14);
SELECT * FROM games WHERE no_of_medals IN(40, 45, 60, 65);

/* between */

SELECT * FROM games WHERE id between 1 AND 8;
SELECT * FROM games WHERE game_name between 'kabbaddi' AND 'swimming';
SELECT * FROM games WHERE host_country between 'india' AND 'japan';
SELECT * FROM games WHERE id between 6 AND 13;
SELECT * FROM games WHERE no_of_medals between 85 AND 95;
